    ...
    public partial class MainWindow : Window
    {
        ...
        private async void Search_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                foreach (var ticker in tickers)
                {
                    // sle note: there is no await so the running tasks to get stock tickers are placed into a task List to be collectively awaited just below.
                    var loadTask = service.GetStockPricesFor(ticker, cancellationTokenSource.Token);
                ...
                // sle note: at this point the all the stock have been merged into an array, all tasks have completed, now time for the parallel processing demonstration.
                decimal total = 0;

                Parallel.ForEach(loadedStocks, (stocks) =>
                {
                    // sle note: this code is is placed, behind the scenes into a task and runs to completion.
                    var value = 0m;
